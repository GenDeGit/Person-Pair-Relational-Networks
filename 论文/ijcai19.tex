...
All of previous works, a relationship between two people is predicted independently, and the interaction between those relationships, which can help the prediction, are ignored.
...
what's more, we made a detail analysis, and found that
...
\begin{figure}[htpb]
	...
	%\vspace*{-0.08cm}
 \caption{An example of an image and its social relation graph from PIPA-relation.
...
However, it is still challenging to infer a unknown social relationship between two persons in a image.
...
The previous works are incur the contextual objects region, and these contextual region is not always correctly detected by extra annotation. So, whether to introduce the information of the objects in scene has also became an important consideration for us. Specifically, the proposing method should avoiding introduce uncertain information, and the approach also model the interaction of various person pairs in an image well.
...
\begin{table}[htpb]
  ...
  \caption{Statistic on PISC and PIPA-relation. ``Sui.'' denotes that the percentage of person pair in an image with multiple person pairs. while ``Unsui.'' is its opposite. ``Single Rel.'' represents proportion of the images that have only one category of social relationship, while ``Multi Rel.'' is its opposite (in \%).}
...
In this subsection, we made an analysis on PISC and PIPA-relation. Here we only used its train set and test set for statistic. As shown in table \ref{tab:data_analysis}, we first counted the relationship categories of each image. Specifically, on PISC coarse, 79.9\% of images have only one coarse category, while 20.1\% images have multiple coarse one. Second, 87.1\% and 83.9\% person pairs are the proportion that suitable for interaction in PISC coarse and PISC fine datasets.
...
On this dataset, we also compare our proposed model with the existing methods,i.e, Two stream CNN\cite{DBLP:conf/cvpr/SunSF17},Dual-glance\cite{DBLP:conf/iccv/LiWZK17} and GRM\cite{DBLP:conf/ijcai/WangCRYCL18} that achieves the best performance before. Specifically, we directly reprint the experimental of several baselines from the literature. The result are presented in Table \ref{tab:pipa_table}. Notably, our {\PPRN} significantly outperforms previous methods. Still, our model outperforms all the baselines in PIPA-relation, and beating the best of them 2.4\%.
...
In this section, we first present the comparison result  of message passing mechanism and analysis the reasons behind, and the result present in Table \ref{tab:result_ana}.
...
As table \ref{tab:result_ana} reported, RCNN, which uses only object information, has the worst effect. \cite{DBLP:conf/iccv/LiWZK17} trained a model with attention mechanisms to exploit different object regions cues according different pairs of people. we also conduct a attention mechanisms as same as Dual-glance, but the performance do not improved as reported in Table \ref{tab:result_ana}.

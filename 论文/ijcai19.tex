...
Four examples in Figure \ref{fig:case_study} are shown to illustrate the capacity of our PRN to infer social relationshps. We compare two predicted social relationships, one from our PRN and the another from GRM \cite{DBLP:conf/ijcai/WangCRYCL18}. We find that compared to the social relation graph in (c), the graph in (b) in all these examples are both much similar to the graph in (a), which means that our PRN performs better than GRM. Additionally, the edges in (a) are almost consistent, meaning that the social relationships in a still image are stable. Considering the first example, the true social relationship between the person with an orange box and the another with an green one is No Relation, which can be correctly predicted by our PRN while wrongly predicted as Couple by GRM.

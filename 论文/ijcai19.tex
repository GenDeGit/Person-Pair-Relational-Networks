...
\begin{table}[htpb]
  ...
  \caption{Statistic on PISC and PIPA-relation. ``Sui.'' denotes that the percentage of person pair in an image with multiple person pairs. while ``Unsui.'' is its opposite. ``Single Rel.'' represents the images' proportion that has only one category of social relationship, while ``Multi Rel.'' is its opposite (in \%).}
...
In this subsection, we made an analysis on PISC and PIPA-relation. Here we only used its train set and test set for statistic. As shown in table \ref{tab:data_analysis}, we first counted the social relationship categories of each image. Specifically, on PISC coarse, 79.9\% of images have only one coarse category, while 20.1\% images have multiple coarse one. Second, 87.1\% abd 83.9\% are  the proportion that suitable for interaction of person pairs.

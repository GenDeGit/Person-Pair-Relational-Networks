...
Formally, given the current GRU hidden states of node $h_i$, we takes the messages from other nodes as $m_{i,j \to i}$, and $h_{j \to i}$ as the hidden state of other node. $m_{i,j \to i}$ is computed by function of its hidden state $h_i$.To be more specific, $m_{i, j \to i}$ are computed by the following message pooling functions:

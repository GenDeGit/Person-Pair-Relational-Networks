...
\begin{figure}[htpb]
	...
	%\vspace*{-0.08cm}
	\caption{An example image from PIPA dataset. There are 4 person-pair social relationships in the picture: friends, friends, friends and band members.}
	\vspace*{-3.5mm}
	\label{example}
\end{figure}

As one example in PIPA(Figure \ref{example}) where the picture denotes a scene, there are 3  "friends" relationships and 1 "band members" relationship in the scene. If we know only that two of these relationships are "friends, "we can infer that it is most likely a group of friends, and thus we infer that the unknown relationship is "friends" or related to "friends". In other words, we can easily use the interaction of different relationships in the same scene to detect each social relationship. Therefore we address the issue with focusing more on the interaction between every social relationships in one scene to improve the social relationship detection performance.
...
\begin{figure*}[htpb]
	\centering
	%	\includegraphics[width=0.48 \textwidth, trim=10 10 10 80,clip]{./pic/example_new.pdf}
	\includegraphics[width=0.96 \textwidth,clip]{./pic/model.png}
	%\hspace{0.02\textwidth}
	%\vspace*{-0.08cm}

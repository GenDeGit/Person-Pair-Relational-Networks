...
What's more, we analyze PISC and PIPA-relation datasets (\ref{section:dataset_anaysis}), and found that in many images, the relationships of person pairs in the same image tend to be similar.
...
Similar to the model of GRM, we also adopt the per-class recall and mean average precision(mAP) to evaluate our model. The experiments data are reported in Table \ref{tab:pisc_table}. First, both Pair CNN + BBox + Global, Dual-glance and GRM are incurred extra Faster-RCNN\cite{DBLP:conf/nips/RenHGS15} to extract the local contextual cues(object proposal). GRM utilized the object proposal to construct a semantic-aware  knowledge graph for reason about the social relationship. It is notable that both of them incur extra detection annotations that contain noises. Specifically, our model achieves the accuracy of 75.1\% and mAP of 81.8\% for the coarse-level recognition. The model also takes an accuracy of 65.6\% and mAP of 69.7\% for the fine-level dataset. Our model beats the previous best model in the fine-level recognition ,but slightly lower on coarse-level than the best model before.
